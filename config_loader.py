...
def _get_typed_env_value(key: str, default: Any, value_type: type) -> Any:
    ...
    try:
        ...
        logger.error(
            f"Invalid value for {key}: '{str_value}'. Expected type {value_type.__name__}. Error: {e}. Using default: {default}"
        )
        return default
...
def load_config_values() -> dict[str, Any]:
    ...
    config["SESSION_FILE"] = _get_typed_env_value(
        "SESSION_FILE", "session_data.json", str
    )  # Keep as string, Path conversion later if needed
    ...
    config["CANDIDATE_PROFILE_PATH"] = _get_typed_env_value(
        "CANDIDATE_PROFILE_PATH", "candidate_profile.json", str
    )
    config["MATCHING_MODEL_TIER1"] = _get_typed_env_value(
        "MATCHING_MODEL_TIER1", "gpt-4o-mini", str
    )
    config["MATCHING_THRESHOLD_TIER1"] = _get_typed_env_value("MATCHING_THRESHOLD_TIER1", 60, int)
    config["MATCHING_MODEL_TIER2"] = _get_typed_env_value(
        "MATCHING_MODEL_TIER2", "gpt-4.1-mini", str
    )
    ...
    for key in ["MATCHING_THRESHOLD_TIER1", "MATCHING_THRESHOLD_FINAL"]:
        ...
        if not (0 <= threshold <= 100):
            logger.warning(
                f"Configuration value for {key} ({threshold}) is outside the valid range [0, 100]. Clamping."
            )
    ...
    if config["MATCHING_ENABLED"]:
        logger.info(
            f"AI Job Matching Enabled: Tier1='{config['MATCHING_MODEL_TIER1']}' (Threshold {config['MATCHING_THRESHOLD_TIER1']}), Tier2='{config['MATCHING_MODEL_TIER2']}' (Final Threshold {config['MATCHING_THRESHOLD_FINAL']})"
        )
