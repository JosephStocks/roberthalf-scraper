...
def send_pushover_notification(
    ...
    if user not in USER_KEYS and user != "All":
        logger.error(
            f"Invalid Pushover user specified: '{user}'. Must be 'Joe', 'Katie', or 'All'."
        )
    ...
    if user == "All":
        ...
        if not user_key:
            logger.error(
                f"User key for '{user}' (PUSHOVER_USER_KEY_{user.upper()}) is not set. Cannot send notification."
            )
            return
    ...
    if len(message.encode("utf-8")) > 1024:  # Check byte length
        logger.warning(
            "Message length is potentially too long for Pushover (>1024 bytes), truncating."
        )
    ...
    if "monospace" in data:
        ...
        if data.get("html") == 1 and data.get("monospace") == 1:
            logger.warning(
                "Both 'html' and 'monospace' flags set for Pushover, disabling 'monospace'."
            )
            data["monospace"] = 0
    ...
    try:
        ...
        logger.debug(
            f"Pushover payload (excluding token/user keys): "
        ...
        if e.response is not None:
            try:
                ...
                error_details = (
                    f"Status {status}, User Invalid: '{user_error}', Errors: {', '.join(errors)}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--html", action="store_true", help="Enable HTML formatting in the message."
    )
