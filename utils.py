...
def get_proxy_config() -> ProxySettings | None:  # Changed return type
    """
    ...
    if not proxy_server or not proxy_auth:
        logger.warning(
            "Proxy configuration (PROXY_SERVER, PROXY_AUTH) is incomplete. Proxy disabled."
        )
        return None
    ...
    try:
        ...
        logger.info(
            f"Proxy enabled: Server={config['server']}, User={config['username']}, Bypass={config.get('bypass', 'N/A')}"
        )
        ...
        logger.error(
            "Error parsing PROXY_AUTH. Expected format 'username:password'. Proxy disabled."
        )
        return None
    except Exception as e:
        # Catch any other unexpected errors during config creation
        logger.error(
            f"Unexpected error creating proxy configuration: {e}. Proxy disabled.", exc_info=True
        )  # Log traceback
        return None
