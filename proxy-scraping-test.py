...
def scrape_with_iproyal_proxy():
    ...
    if not proxy_config:
        logger.error(
            "Proxy is required for this test but is disabled or misconfigured in .env.test. Exiting."
        )
    ...
    try:
        ...
        )  # Optional headless for testing
        logger.info(
            f"Test Config: Max Req/IP={max_requests_per_ip}, Max Time/IP={max_time_per_ip}s, Delay={request_delay}s, Headless={headless_mode}"
        )
    except ValueError as e:
        logger.error(
            f"Invalid non-integer value for test config (MAX_REQUESTS_PER_IP, MAX_TIME_PER_IP, REQUEST_DELAY): {e}"
        )
        return
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            logger.info(
                f"Launching browser with proxy: {proxy_config['server']} (Headless: {headless_mode})"
            )
            browser = p.chromium.launch(proxy=proxy_config, headless=headless_mode)
            ...
            while True:
                ...
                try:
                    ...
                    if request_count >= max_requests_per_ip or (
                        ...
                        logger.info(
                            f"Rotation condition met (Requests: {request_count}/{max_requests_per_ip}, Time: {time.time() - start_time:.1f}/{max_time_per_ip}s). Recreating context..."
                        )
                        page.close()
                        context.close()
                    ...
                    logger.error(
                        f"Playwright error during request {request_count + 1} (IP: {current_ip}): {e}"
                    )
                    ...
                    logger.error(
                        f"Unexpected error during request {request_count + 1} (IP: {current_ip}): {e}",
                        exc_info=True,
                    )
